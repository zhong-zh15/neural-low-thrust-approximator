    ...
    int nrhs, const mxArray* prhs[])
{
    ...
    if (nrhs != 2) {
        mexErrMsgIdAndTxt("nn_lowthrust:fast_predict_vector_tmin:InvalidNumInputs",
            "Two inputs required: model_dir (string) and input_vector.");
    }
    if (nlhs > 1) {
        mexErrMsgIdAndTxt("nn_lowthrust:fast_predict_vector_tmin:InvalidNumOutputs",
    ...
    // Create predictor and run fast_predict_vector_tmin
    EigenFastPredictor predictor(modelDir);
    double tmin = predictor.fast_predict_vector_tmin(processed);

    // Return scalar result
    plhs[0] = mxCreateDoubleScalar(tmin);
}
